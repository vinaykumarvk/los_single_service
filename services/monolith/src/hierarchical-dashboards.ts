 ...
 */
async function getDirectReportees(supabaseClient: SupabaseClient, managerId: string): Promise<Array<{
  user_id: string;
  username: string;
  email: string;
  designation: string;
 ...
 */
async function getAllSubordinates(supabaseClient: SupabaseClient, managerId: string): Promise<string[]> {
...
async function computeManagerMetrics(
  ...
  try {
    ...
    const subordinateIds = await getAllSubordinates(supabaseClient, managerId);
    ...
    if (includeReportees) {
      const directReportees = await getDirectReportees(supabaseClient, managerId);
      ...
      const reporteesWithMetrics = await Promise.all(
        directReportees.map(async (reportee) => {
          ...
          const directReportees = await getDirectReportees(supabaseClient, reportee.user_id);
          if (directReportees.length > 0 && reportee.designation !== 'Relationship Manager') {
            ...
            reporteeMetrics = await computeManagerMetrics(supabaseClient, reportee.user_id, false);
          } else {
            reporteeMetrics = await computeUserMetrics(supabaseClient, reportee.user_id);
          }
          return {
 ...
 */
export function setupHierarchicalDashboards(app: Express, supabaseClient: SupabaseClient) {
  ...
  app.get('/api/dashboard/rm/:userId', async (req: Request, res: Response) => {
    try {
      ...
      const metrics = await computeUserMetrics(supabaseClient, userId);
  ...
  app.get('/api/dashboard/srm/:srmId', async (req: Request, res: Response) => {
    try {
      ...
      const metrics = await computeManagerMetrics(supabaseClient, srmId, includeReportees);
  ...
  app.get('/api/dashboard/regional-head/:headId', async (req: Request, res: Response) => {
    try {
      ...
      const includeReportees = req.query.includeReportees === 'true';
      const metrics = await computeManagerMetrics(supabaseClient, headId, includeReportees);
  ...
  app.get('/api/hierarchy/reportees/:managerId', async (req: Request, res: Response) => {
    try {
      ...
      const reportees = await getDirectReportees(supabaseClient, managerId);
      ...
      const reporteesWithMetrics = await Promise.all(
        reportees.map(async (reportee) => {
          ...
          // If reportee is a manager (has their own reportees), compute manager metrics
          const hasSubordinates = (await getAllSubordinates(supabaseClient, reportee.user_id)).length > 0;
          if (hasSubordinates) {
            metrics = await computeManagerMetrics(supabaseClient, reportee.user_id, false);
          } else {
            metrics = await computeUserMetrics(supabaseClient, reportee.user_id);
          }
          return {
            userId: reportee.user_id,
            username: reportee.username,
